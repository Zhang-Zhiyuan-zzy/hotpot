...
测试矩阵按风险机制组合，而不是对每个化学身份做无意义的笛卡尔积：

- 上表全部身份覆盖隐式氢输入；
- 显式羟基氢、芳香氢和显式双齿配体作为“已有氢不得删除”的代表；
- 水/醇、阴离子 O、普通 N、芳香 N、羰基 O 覆盖不同价态分支；
- 单齿案例与至少一个双齿案例分别覆盖；
- 代理或最终优化失败时，本体的氢、形式电荷和键集合完全不变；
- 新标准接口不再接受 `rm_polar_hs`；该名称按未知参数显式报错，不再测试
  `True/False` 两条会删除氢的旧业务路径。
