...
其中 I 和 III 是 IV 的计算前提；II 不是。一个高应变但边界清楚的环仍然可以执行穿环判断，
一个能量合理但高度折叠、无法稳定定义环孔的环反而可能无法得到唯一穿环结论。
...
`Q < 1` 表示有效体积碰撞，`Q ≈ 1` 表示临界接触。`rho` 是需要按元素、键型和业务数据
标定的碰撞半径，不能未经验证直接等同于完整范德华半径。
...
这三类检查互不替代。1–3、1–4 原子、氢键、金属配位等局部关系需要单独的拓扑排除或
参数表。
...
静态几何无法严格证明“物理上现实”。建议将下列项目作为化学条件化的软评分：

- 键长、键角、二面角和 improper torsion 相对参考分布的 z-score；
- 芳香/共轭局部的离面程度；
- Cremer–Pople puckering coordinates；
- 非键排斥能、总应变或最大力；
- 轻度约束优化后的 RMSD、最大原子位移和拓扑是否保持。

参考分布至少需要区分元素、键级、局部配位、芳香性、电荷和环尺寸。力场无参数或超出适用
域时应报告 `UNRESOLVED`，不能把低质量能量静默当成可靠物理证据。

Cremer–Pople 坐标能稳定描述 chair、boat、twist 等环起伏模态，但只回答“形状是什么”，
不自动回答“是否合理”。参考：Cremer and Pople, JACS 1975,
...
- 化学类型条件化的键长、键角、扭转和 puckering 分布；
- 力场最大力及轻度松弛稳定性；
- 坐标微扰后的分类稳定性；
- knot/link 类型和预期拓扑保持。
