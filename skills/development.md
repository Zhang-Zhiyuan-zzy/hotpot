...
- 只有当用户明确说出“将暂存规则整理并更新development”时，才允许修改 `development.md` 和 `development.en.md` 正式版。
- 其他“记录”、“新增”、“修订”或“暂存”开发规则的请求只能更新 `development.tmp.md`，不得据此改动两份正式文档。
- 获得上述明确授权后，规则并入 MUST 同时重构中英文两份正式文档，合并重复内容，并删除过时或不再适用的规则。
