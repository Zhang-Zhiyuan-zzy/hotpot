...
class MachineLearning:
    ...
    def __init__(
        ...
        Args:
            ...
            features (Sequence|np.ndarray): feature columns in ChemData DataFrame
            target: target column in ChemData DataFrame
    ...
    def calc_shap_values(
        ...
        Args:
            ...
            X: input ChemData to train estimator
            y: target ChemData to train estimator
            feature_names: feature names for each column of input ChemData
            sample_size: how many samples sample from input ChemData to carry out SHAP analysis
            X_test: ChemData to be explained by SHAP explainer. if not given, test ChemData is generated by
        ...
        # Save result, base, ChemData value and SHAP value
        if shap_values_save_path:
            ...
            with pd.ExcelWriter(shap_values_save_path) as writer:
                ...
                df_data_value.to_excel(writer, sheet_name='ChemData')
...
class MachineLearning_:
    ...
    def __init__(
        ...
        Keyword Args:
            ...
            hyper_optimizer:
            dir_exists (bool, optional): whether to allow stored ChemData in an existing directory.
        """
        self.work_dir = Path(work_dir)
        self.picture_dir = self.work_dir.joinpath('pictures')
        self.sheet_dir = self.work_dir.joinpath('sheet')
    ...
    def _check_feature_names(self, features: Sequence[str]) -> list[str]:
        ...
        for name in features:
            if name not in sheet_title:
                raise ValueError(f"The feature {name} not in ChemData")
    ...
    def invert_scaling_X(
        ...
        """
        Inversely transforms the scaled ChemData `X` back to its original scale.
        ...
        ----------
        X : numpy.ndarray
            The ChemData matrix to be inverse-transformed.
        ...
        -------
        numpy.ndarray
            The ChemData `X` transformed back to its original scale. If `recovery_cluster_to_original`
        ...
        if features is None:
            ...
            # Initialize lists for indices and ChemData
            inv_features = []
            indices_in_ori = []
    ...
    #         ChemData: Union[pd.DataFrame, np.ndarray],
    #         test_indices: Union[Sequence[int], np.ndarray],
    #         highlight_indices: Union[Sequence[int], np.ndarray] = None,
    #         columns: Sequence = None
    # ):
    #     """"""
    #     if isinstance(ChemData, np.ndarray):
    #         if isinstance(columns, Sequence):
    #             ChemData = pd.DataFrame(ChemData, columns=list(columns))
    #         else:
    #             ChemData = pd.DataFrame(ChemData, columns=[f'f{i}' for i in range(ChemData.shape[1])])
    #
    #     elif isinstance(ChemData, pd.DataFrame):
    #         if isinstance(columns, Sequence):
    #             ChemData.columns = columns
    #
    #     else:
    #         raise TypeError('the ChemData should be a pandas dataframe or a numpy array!')
    #
    ...
    def train_surrogate_tree_(
        ...
        Args:
            ...
            X (np.ndarray): Input features for training or as a template for ChemData generation.
            ...
            generate_train_data (bool, optional): If `True`, generates new training ChemData using `generator_kwargs`. Defaults to `False`.
            generate_valid_data (bool, optional): If `True`, generates new validation ChemData using `generator_kwargs`. Defaults to `True`.
            generator_kwargs (dict, optional): Arguments for the ChemData generation function. Defaults to `None`.
